...
void Event::setEvent(Tool* tool)
{
	...
	for(unsigned int iT=0; iT<m_nTrk; iT++)
	{
		...
		double trkEffCrc = 0.06*(tool->detTrkEff(m_fcalEt,m_trkPt[iT],m_trkEta[iT])-tool->detTrkEff(m_fcalEt,0.5,m_trkEta[iT]))/(tool->detTrkEff(m_fcalEt,5.0,m_trkEta[iT])-tool->detTrkEff(m_fcalEt,0.5,m_trkEta[iT]))-0.03;
		if(trkEffCrc>+0.1) trkEffCrc = +0.1;
		if(trkEffCrc<-0.1) trkEffCrc = -0.1;
		double trkEff = tool->detTrkEff(m_cent,m_trkPt[iT],m_trkEta[iT]) + trkEffCrc; // high efficiency
		...
		if(m_trkQual[iT]!=cutQual) continue;
