...
void mergePt()
{
	...
	TH3D* matchedRecoNorm = (TH3D*)fIn->Get("matchedRecoNorm");
	...
	double binX[NX+1];
	double binY[18] = {0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.2, 1.4, 1.6, 1.8, 2.0, 2.5, 3.0, 4.0, 5.0, 7.5, 10., 20.}; // FIX
	double binZ[NZ+1];
	...
	TH3D* new_allTrue = new TH3D("allTrue","",NX,binX,17,binY,NZ,binZ);
	TH3D* new_allReco = new TH3D("allReco","",NX,binX,17,binY,NZ,binZ);
	TH3D* new_matchedRecoNorm = new TH3D("matchedReco","",NX,binX,17,binY,NZ,binZ);
	TH3D* new_unmatchedRecoToPrimary = new TH3D("unMatchedReco","",NX,binX,17,binY,NZ,binZ);
	...
	for(unsigned iX=0; iX<NX; iX++)
	{
		for(unsigned int iZ=0; iZ<NZ; iZ++)
		{
			for(unsigned int iB=0; iB<17; iB++)
			{
				double sum = 0;
